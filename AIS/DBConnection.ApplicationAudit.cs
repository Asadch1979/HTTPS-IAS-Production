...
namespace AIS.Controllers
    ...
    public partial class DBConnection
        ...
        public void LogApplicationActivity(ApplicationAuditEvent e, ApplicationAuditContext c)
            ...
            AddAuditParameter(cmd, "P_EVENT_TYPE", OracleDbType.Varchar2, V(e.EventType ?? "BUSINESS_ACTION", 50));
            AddAuditParameter(cmd, "P_ACTION_NAME", OracleDbType.Varchar2, V(e.ActionName, 250));
            AddAuditParameter(cmd, "P_ACTION_CATEGORY", OracleDbType.Varchar2, V(e.ActionCategory, 100));
            AddAuditParameter(cmd, "P_MODULE_NAME", OracleDbType.Varchar2, V(e.ModuleName, 100));
            AddAuditParameter(cmd, "P_PPNO", OracleDbType.Varchar2, V(c.Ppno, 30));
            ...
            AddAuditParameter(cmd, "P_SESSION_ID", OracleDbType.Varchar2, V(c.SessionId, 100));
            AddAuditParameter(cmd, "P_PAGE_ID", OracleDbType.Int32, c.PageId);
            AddAuditParameter(cmd, "P_CONTROLLER_NAME", OracleDbType.Varchar2, V(c.ControllerName, 150));
            AddAuditParameter(cmd, "P_CONTROLLER_ACTION", OracleDbType.Varchar2, V(c.ControllerAction, 150));
            AddAuditParameter(cmd, "P_API_PATH", OracleDbType.Varchar2, V(c.ApiPath, 500));
            AddAuditParameter(cmd, "P_HTTP_METHOD", OracleDbType.Varchar2, V(c.HttpMethod, 20));
            AddAuditParameter(cmd, "P_DB_PACKAGE_NAME", OracleDbType.Varchar2, V(e.DbPackageName, 100));
            AddAuditParameter(cmd, "P_DB_PROCEDURE_NAME", OracleDbType.Varchar2, V(e.DbProcedureName, 150));
            ...
            AddAuditParameter(cmd, "P_PROXY_IP_ADDRESS", OracleDbType.Varchar2, V(c.ProxyIp, 100));
            AddAuditParameter(cmd, "P_USER_AGENT", OracleDbType.Varchar2, V(c.UserAgent, 1000));
            AddAuditParameter(cmd, "P_TRACE_ID", OracleDbType.Varchar2, V(c.TraceId, 150));
            AddAuditParameter(cmd, "P_REQUEST_ID", OracleDbType.Varchar2, V(c.RequestId, 150));
        ...
        private static string V(string value, int maximumLength)
            => ApplicationAuditLogger.Truncate(value, maximumLength);
        }
    }
