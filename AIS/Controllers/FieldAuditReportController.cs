...
namespace AIS.Controllers
    {
    public class PdfStatisticsRowRequest
        {
    ...
    public class PdfStatisticsSaveRequest
        {
        public int EngId { get; set; }
        public int ReportVersion { get; set; }
        public List<PdfStatisticsRowRequest> Rows { get; set; } = new List<PdfStatisticsRowRequest>();
        }

    public class IncomeLeakageRowRequest
        {
    ...
    public class IncomeLeakageSaveRequest
        {
        public int EngId { get; set; }
        public int ReportVersion { get; set; }
        public List<IncomeLeakageRowRequest> Rows { get; set; } = new List<IncomeLeakageRowRequest>();
        }
    ...
    public class FieldAuditReportController : Controller
        ...
        public IActionResult GetPdfStatistics(int engId, int reportVersion)
            ...
            var rows = _dbConnection.GetPdfStatistics(engId, normalizedVersion);
            var locked = rows.Any(row => row != null
                                         && (row.Reported.HasValue
                                             || row.Rectified.HasValue
                                             || row.Outstanding.HasValue
        ...
        [HttpPost]
        public IActionResult SavePdfStatistics([FromBody] PdfStatisticsSaveRequest request)
            {
            var redirect = EnsureAuthorized();
            if (redirect != null)
                {
            ...
            if (request == null || request.EngId <= 0)
                {
                return BadRequest(new { success = false, message = "Invalid statistics payload." });
                }
            ...
            foreach (var row in request.Rows ?? new List<PdfStatisticsRowRequest>())
                {
                if (row == null)
                    {
                    continue;
                ...
                if ((row.Reported.HasValue && row.Reported.Value < 0)
                    ...
                    {
                    return BadRequest(new { success = false, message = "Statistics values must be non-negative." });
                    }
                }

            var normalizedVersion = NormalizeReportVersion(request.ReportVersion);
            var userPpNo = _sessionHandler.GetUser()?.PPNumber ?? string.Empty;
            _dbConnection.SavePdfStatistics(request.EngId, normalizedVersion, request.Rows, userPpNo);

            return Json(new { success = true, message = "Saved successfully", locked = true });
            }
        ...
        public IActionResult GetIncomeLeakage(int engId, int reportVersion)
            ...
            var normalizedVersion = NormalizeReportVersion(reportVersion);
            var rows = _dbConnection.GetIncomeLeakage(engId, normalizedVersion);
            var locked = rows.Any(row => row != null
        ...
        [HttpPost]
        public IActionResult SaveIncomeLeakage([FromBody] IncomeLeakageSaveRequest request)
            {
            var redirect = EnsureAuthorized();
            if (redirect != null)
                {
            ...
            if (request == null || request.EngId <= 0)
                {
                return BadRequest(new { success = false, message = "Invalid income leakage payload." });
                }
            ...
            foreach (var row in request.Rows ?? new List<IncomeLeakageRowRequest>())
                {
                if (row == null)
                    {
                    continue;
                ...
                if (row.Amount.HasValue && row.Amount.Value < 0)
                    {
                    return BadRequest(new { success = false, message = "Income leakage amount must be non-negative." });
                    }
            ...
            var normalizedVersion = NormalizeReportVersion(request.ReportVersion);
            var userPpNo = _sessionHandler.GetUser()?.PPNumber ?? string.Empty;
            _dbConnection.SaveIncomeLeakage(request.EngId, normalizedVersion, rows, userPpNo);
        ...
        public IActionResult GetOverallConclusion(int engId, int reportVersion)
            ...
            var normalizedVersion = NormalizeReportVersion(reportVersion);
            var data = _dbConnection.GetOverallConclusion(engId, normalizedVersion) ?? new OverallConclusionRequest();
            var locked = !string.IsNullOrWhiteSpace(data.OverallConclusionHtml)
                         || !string.IsNullOrWhiteSpace(data.NonAddressableHtml)
                         || !string.IsNullOrWhiteSpace(data.FraudProneHtml)
                         || !string.IsNullOrWhiteSpace(data.RegulatoryHtml)
                         || !string.IsNullOrWhiteSpace(data.SafetySecurityHtml);

            return Json(new { success = true, data, locked });
            }

        [HttpPost]
        public IActionResult SaveOverallConclusion([FromBody] OverallConclusionRequest request)
            {
            var redirect = EnsureAuthorized();
            if (redirect != null)
                {
            ...
            if (request == null || request.EngId <= 0)
                {
                return BadRequest(new { success = false, message = "Invalid conclusion payload." });
                }
            ...
            var normalizedVersion = NormalizeReportVersion(request.ReportVersion);
            var userPpNo = _sessionHandler.GetUser()?.PPNumber ?? string.Empty;
            _dbConnection.SaveOverallConclusion(request.EngId, normalizedVersion, request, userPpNo);
        ...
        private static int NormalizeReportVersion(int reportVersion)
            {
            return reportVersion > 0 ? reportVersion : 1;
            }
