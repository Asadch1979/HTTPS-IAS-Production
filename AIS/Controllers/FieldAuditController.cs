...
namespace AIS.Controllers
    ...
    public class FieldAuditController : Controller
        ...
        [HttpGet]
        public IActionResult BO_Dashbaord()
            {
            ViewData["TopMenu"] = _topMenus.GetTopMenus();
            ViewData["TopMenuPages"] = _topMenus.GetTopMenusPages();
            ViewData["HideTopHeader"] = true;
            ...
            var model = new ExecutionDashboardViewModel
                {
                Steps = BuildBODashboardSteps()
                };

            return View("~/Views/FieldAudit/BO_Dashbaord.cshtml", model);
            }

        [HttpGet]
        public IActionResult LoadBODashboardStep(string step, int engId, bool isReadOnly = false)
            {
            if (!User.Identity.IsAuthenticated)
                {
                return Unauthorized();
            ...
            switch ((step ?? string.Empty).Trim().ToUpperInvariant())
                {
                case "DRAFT_REPORT":
                    return PartialView("~/Views/FieldAudit/BC_Partials/_DraftReportPartial.cshtml");
                case "QUALITY_REVIEW":
                    return PartialView("~/Views/FieldAudit/BC_Partials/_QualityReviewPartial.cshtml");
                case "ISSUE_REPORT":
                    ViewData["EntitiesList"] = _dbConnection.GetAuditConcludingEntities();
                    return PartialView("~/Views/FieldAudit/BC_Partials/_IssueReportPartial.cshtml");
                case "CHECKING_DRAFT_REPORT":
                    ViewData["ExecutionDashboardReadOnly"] = true;
                    return PartialView("~/Views/FieldAudit/BC_Partials/_CheckingDraftReportPartial.cshtml");
                case "CHECKING_QUALITY_REVIEW":
                    ViewData["ExecutionDashboardReadOnly"] = true;
                    return PartialView("~/Views/FieldAudit/BC_Partials/_CheckingQualityReviewPartial.cshtml");
                default:
                    return BadRequest("Invalid workflow step.");
                }
            }

        private List<ExecutionDashboardStepViewModel> BuildBODashboardSteps()
            {
