...
namespace AIS.Services
    ...
    public class FieldAuditReportPdfBuilder
        ...
        public string BuildHtml(FieldAuditPdfReportData data)
            ...
            sb.AppendLine(".para-body table{ width:100% !important; max-width:100% !important; border-collapse:collapse; margin-top:10px; box-sizing:border-box; }");
            sb.AppendLine(".para-body table.report-table-bordered{ table-layout:fixed !important; }");
            sb.AppendLine(".para-body table.report-table-borderless{ table-layout:auto !important; }");
            sb.AppendLine(".para-body table.report-table-bordered th,.para-body table.report-table-bordered td{ border:1px solid #222; padding:4px 6px; vertical-align:top; word-break:break-word; overflow-wrap:anywhere; white-space:normal !important; }");
            sb.AppendLine(".para-body table.report-table-borderless th,.para-body table.report-table-borderless td{ border:none; padding:4px 6px; vertical-align:top; word-break:normal; overflow-wrap:normal; white-space:normal !important; }");
            ...
            sb.AppendLine(".para-detail table{ width:100% !important; max-width:100% !important; border-collapse:collapse !important; }");
            sb.AppendLine(".para-detail table.report-table-bordered{ table-layout:fixed !important; }");
            sb.AppendLine(".para-detail table.report-table-borderless{ table-layout:auto !important; }");
            sb.AppendLine(".para-detail th,.para-detail td{ white-space:normal !important; vertical-align:top !important; padding:3px 4px !important; font-size:10px !important; }");
            sb.AppendLine(".para-detail table.report-table-bordered th,.para-detail table.report-table-bordered td{ word-break:break-word !important; overflow-wrap:anywhere !important; }");
        ...
        private static string NormalizeTableMarkup(string html)
            {
            var tableTags = FindTableTags(html);
            if (tableTags.Count == 0)
                {
                return html;
                }

            if (HasNestedTables(tableTags))
                {
                return html;
                }
            ...
            for (var i = 0; i < tableTags.Count; i++)
                {
                var tag = tableTags[i];
                sb.Append(html, lastIndex, tag.StartIndex - lastIndex);
                var tagText = html.Substring(tag.StartIndex, tag.Length);

                if (tag.IsOpening)
                    {
                    var tableSegment = ExtractTableSegment(html, tableTags, i);
                    var isBordered = IsBorderedTable(tagText, tableSegment);
                    var updatedTag = AddClassToTag(tagText, "report-table");
                    updatedTag = AddClassToTag(updatedTag, isBordered ? "report-table-bordered" : "report-table-borderless");
                ...
                lastIndex = tag.StartIndex + tag.Length;
                }
        ...
        private static List<TableTag> FindTableTags(string html)
            {
            var tags = new List<TableTag>();
            var index = 0;

            while (index < html.Length)
                {
                var openIndex = html.IndexOf("<table", index, StringComparison.OrdinalIgnoreCase);
                var closeIndex = html.IndexOf("</table", index, StringComparison.OrdinalIgnoreCase);
                var tagIndex = openIndex == -1 ? closeIndex : closeIndex == -1 ? openIndex : Math.Min(openIndex, closeIndex);
                ...
                var endIndex = html.IndexOf('>', tagIndex);
                if (endIndex < 0)
                    {
                    break;
                    }
                ...
                tags.Add(new TableTag
                    {
                    StartIndex = tagIndex,
                    Length = endIndex - tagIndex + 1,
                    IsOpening = tagIndex == openIndex
                    });
        ...
        private static bool HasNestedTables(List<TableTag> tags)
            {
            var depth = 0;
        ...
        private static string ExtractTableSegment(string html, List<TableTag> tags, int openTagIndex)
            {
            for (var i = openTagIndex + 1; i < tags.Count; i++)
                {
                if (!tags[i].IsOpening)
                    {
                    var start = tags[openTagIndex].StartIndex;
                    var end = tags[i].StartIndex + tags[i].Length;
                    return html.Substring(start, end - start);
                    }
                }

            return html.Substring(tags[openTagIndex].StartIndex, tags[openTagIndex].Length);
            }

        private static bool IsBorderedTable(string openTag, string tableSegment)
            {
            if (HasBorderAttribute(openTag))
                {
                return true;
                }

            var styleMatch = Regex.Match(openTag, "style\\s*=\\s*([\"'])(?<value>[^\"']*)\\1", RegexOptions.IgnoreCase);
            if (styleMatch.Success && HasVisibleBorder(styleMatch.Groups["value"].Value))
                {
                return true;
                }

            return Regex.IsMatch(tableSegment, "\\bborder\\s*:\\s*(?!0|0px|0pt|none)[^;\"']+", RegexOptions.IgnoreCase);
            }

        private static bool HasBorderAttribute(string tag)
            {
            var match = Regex.Match(tag, "\\bborder\\s*=\\s*([\"']?)(?<value>\\d+)\\1", RegexOptions.IgnoreCase);
            return match.Success && !string.Equals(match.Groups["value"].Value, "0", StringComparison.OrdinalIgnoreCase);
            }

        private static bool HasVisibleBorder(string style)
            {
            var matches = Regex.Matches(style, "border[^:]*:\\s*(?<value>[^;]+)", RegexOptions.IgnoreCase);
            foreach (Match match in matches)
                ...
                if (Regex.IsMatch(value, "\\bnone\\b", RegexOptions.IgnoreCase))
                    {
                    continue;
                    }

                if (Regex.IsMatch(value.Trim(), "^0(px|pt|em|%)?$", RegexOptions.IgnoreCase))
                    {
                    continue;
                    }
        ...
        private static string AddClassToTag(string tag, string className)
            {
            ...
            return tag.Insert(insertIndex, $" class=\"{className}\"");
            }

        private sealed class TableTag
            {
