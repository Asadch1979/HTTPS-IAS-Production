...
namespace AIS.Services
    ...
    public class FieldAuditReportPdfBuilder
        ...
        public string BuildHtml(FieldAuditPdfReportData data)
            ...
            sb.AppendLine("h2, h3 { page-break-after: avoid; break-after: avoid; }");
            ...
            sb.AppendLine(".para-table{ width:100%; border-collapse:separate; border-spacing:0; border:1px solid #d0d7de; border-radius:8px; margin:10px 0; background:#fff; page-break-inside:avoid; break-inside:avoid; }");
            sb.AppendLine(".para-table thead th{ border:none; background:#f6f8fa; font-weight:700; font-size:13px; padding:10px 12px; text-align:left; }");
            sb.AppendLine(".para-table tbody td{ border:none; padding:12px 14px; }");
        ...
        private static void AppendExecutiveSummary(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            if (!HasMeaningfulContent(summaryContent))
                {
                summaryContent = FindSectionContentByTitle(data, "Executive Summary");
                }
        ...
        private static void AppendStaffPosition(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            foreach (var row in rows)
                {
                sb.AppendLine("<tr>");
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(row.Designation));
        ...
        private static void AppendKpiSnapshot(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            if (chartImages.Count == 0)
                {
                chartImages = CollectChartImages(data, "kpi", "chart");
                }
            ...
            if (kpiRows.Count > 0)
                ...
                foreach (var group in groupedKpis.OrderBy(group => group.Key))
                    ...
                    sb.AppendFormat(CultureInfo.InvariantCulture, "<td class=\"left\">{0}</td>", EncodeOrDash(kpiLabel));
        ...
        private static void AppendNplAnalysis(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            if (chartImages.Count == 0)
                {
                chartImages = CollectChartImages(data, "npl", "chart");
                }
        ...
        private static void AppendSignificantParas(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            foreach (var para in significant)
                {
                sb.AppendLine("<tr>");
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(para.ParaNo));
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(para.Gist));
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(para.Nature));
                sb.AppendLine("</tr>");
                }
            sb.AppendLine("</tbody>");
            sb.AppendLine("</table>");
        ...
        private static void AppendAuditStatistics(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            foreach (var row in rows)
                {
                sb.AppendLine("<tr>");
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(row.RiskLevel));
        ...
        private static void AppendIncomeLeakage(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            foreach (var row in rows)
                {
                sb.AppendLine("<tr>");
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(row.CaseReference));
                sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(row.Description));
        ...
        private static void AppendMetaRow(StringBuilder sb, string label, string value)
            ...
            sb.AppendFormat(CultureInfo.InvariantCulture, "<td>{0}</td>", EncodeOrDash(value));
            sb.AppendLine("</tr>");
            }
        ...
        private static void AppendKeyValueParagraph(StringBuilder sb, string label, string value)
            ...
            sb.AppendFormat(CultureInfo.InvariantCulture, "<p><strong>{0}:</strong> {1}</p>", Encode(label), EncodeOrDash(value));
            }
        ...
        private static string FindSectionContentByTitle(FieldAuditPdfReportData data, string title)
            {
            if (data?.Sections == null || string.IsNullOrWhiteSpace(title))
                {
                return null;
                }

            var match = data.Sections.FirstOrDefault(item =>
                string.Equals(item?.SectionTitle, title, StringComparison.OrdinalIgnoreCase));

            if (match == null)
                {
                match = data.Sections.FirstOrDefault(item =>
                    !string.IsNullOrWhiteSpace(item?.SectionTitle)
                    && item.SectionTitle.IndexOf(title, StringComparison.OrdinalIgnoreCase) >= 0
                    && item.SectionTitle.IndexOf("facts", StringComparison.OrdinalIgnoreCase) < 0
                    && item.SectionTitle.IndexOf("conclusion", StringComparison.OrdinalIgnoreCase) < 0);
                }
        ...
        private static string EncodeOrDash(string input)
            {
            return string.IsNullOrWhiteSpace(input) ? "-" : Encode(input);
            }
        ...
        private static void AppendParaTextBlock(StringBuilder sb, string htmlContent)
            ...
            sb.AppendLine("<table class=\"para-table\">");
            sb.AppendLine("<thead>");
            sb.AppendLine("<tr><th>Para Text</th></tr>");
            sb.AppendLine("</thead>");
            sb.AppendLine("<tbody>");
            sb.AppendLine("<tr><td>");
        ...
        private static void AppendParaBodyDetails(StringBuilder sb, FieldAuditPdfParaModel para)
            ...
            sb.AppendLine("<table class=\"grid\">");
            sb.AppendLine("<tbody>");
            AppendGridRow(sb, "Annexure Code", para.AnnexureCode);
            AppendGridRow(sb, "Instances", para.Instances);
            AppendGridRow(sb, "Amount", para.Amount);
            sb.AppendLine("</tbody>");
            sb.AppendLine("</table>");
        ...
        private static void AppendGridRow(StringBuilder sb, string label, string value)
            ...
            sb.AppendFormat(CultureInfo.InvariantCulture, "<td class=\"left\">{0}</td>", EncodeOrDash(value));
            sb.AppendLine("</tr>");
            }
        ...
        private static List<string> CollectChartImages(FieldAuditPdfReportData data, params string[] keywords)
            {
            var images = new List<string>();
            ...
            var keywordList = keywords?.Where(keyword => !string.IsNullOrWhiteSpace(keyword))
                .Select(keyword => keyword.ToLowerInvariant())
                .ToList() ?? new List<string>();

            foreach (var section in data.Sections)
                {
                var label = $"{section?.SectionCode} {section?.SectionTitle}".ToLowerInvariant();
                if (keywordList.Count > 0 && keywordList.Any(keyword => !label.Contains(keyword)))
                    {
                    continue;
                    }

                images.AddRange(ExtractChartImages(section?.HtmlContent));
                }
