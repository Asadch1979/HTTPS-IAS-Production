...
namespace AIS.Services
    ...
    public class FieldAuditReportPdfBuilder
        ...
        public string BuildHtml(FieldAuditPdfReportData data)
            ...
            sb.AppendLine(".meta-grid ul { margin: 0; padding-left: 16px; }");
        ...
        private static void AppendKpiSnapshot(StringBuilder sb, FieldAuditPdfReportData data)
            ...
            if (kpiRows.Count > 0)
                ...
                sb.AppendLine("<thead>");
                sb.AppendLine("<tr>");
                sb.AppendLine("<th rowspan=\"2\" style=\"width:28px\" class=\"num-right\">Sr</th>");
                ...
                foreach (var group in groupedKpis.OrderBy(group => group.Key))
                    ...
                    sb.AppendLine("<tr>");
                    sb.AppendFormat(CultureInfo.InvariantCulture, "<td class=\"num-right\">{0}</td>", index++);
        ...
        private static string BuildTeamMembersList(IEnumerable<GetTeamDetailsModel> auditTeam)
            {
            if (auditTeam == null)
                {
                return string.Empty;
            ...
            foreach (var name in members)
                {
                builder.AppendFormat(CultureInfo.InvariantCulture, "<li>{0}</li>", Encode(name));
                }
            builder.Append("</ul>");
