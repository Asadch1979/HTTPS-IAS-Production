...
(function () {
    ...
    function validateObservationHeading(showMessage) {
        if (window.CommonValidation && CommonValidation.isAlnumOk) {
            CommonValidation.isAlnumOk('#viewMemo_heading', { allowAmp: true, allowQuestion: true, allowComma: true, allowSpace: true, required: true });
        }
    ...
    function attachObservationHeadingRestriction() {
        if (window.CommonValidation && CommonValidation.attachAlnumOnly) {
            CommonValidation.attachAlnumOnly('#viewMemo_heading', {
                allowAmp: true,
                allowQuestion: true,
                allowComma: true,
                allowSpace: true,
    ...
    function applyDefaultValues() {
        ...
        if ($.trim($('#amount_inv_field').val()) === '' || $('#amount_inv_field').val() === '0.00') {
            $('#amount_inv_field').val((amountDefault ?? 0).toString());
        }

        if ($.trim($('#no_instances_field').val()) === '' || $('#no_instances_field').val() === '0') {
            $('#no_instances_field').val((instancesDefault ?? 1).toString());
        }

        if (!$('#viewMemo_replydays').val()) {
            $('#viewMemo_replydays').val((daysDefault ?? 3).toString());
        }
    }
    ...
    function initObservationReplica() {
        ...
        attachObservationHeadingRestriction();
    ...
    window.div_risksubcategoryShowHide = function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (index, item) {
                        $('#riskSubGroupSelectBox').append("<option value=\"" + escapeHtml(item.s_ID) + "\"> " + escapeHtml(item.heading) + " </option > ");
                    });
    ...
    window.div_activityContainerShowHide = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    $('#riskActivitiesSelectBox').append("<option value=\"" + escapeHtml(item.id) + "\"> " + escapeHtml(item.heading) + "</option>");
                });
    ...
    window.submitObservationToAuditee = function () {
        ...
        if ($('#no_instances_field').val() == "") {
            alert('Please Enter No. of Instances, in case of blank please enter 1');
            return;
        }
    ...
    window.openResponsiblePPs = function () {
    ...
    window.deleteRespRow = function (e) {
    ...
    window.updateRespRow = function (e) {
    ...
    window.addResponsibilityToMainTable = function (action) {
