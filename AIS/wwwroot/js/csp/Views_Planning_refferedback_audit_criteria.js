    ...
    function recommendReferredBackAuditCriterias(id, period_id, ent_id, size_id, risk_id, freq_id, visit, nodays, entityId, entName) {
        g_trId = id;
        $('#recommendReferredBackAuditCriteriasModal').modal('show');
        if (ent_id == 25) {
            $('#auditCriteriaCADHUBField').append('<option selected="selected" id="' + entityId + '">' + entName + '</option>');
    ...
    function finalRecommendProcessTransaction() {
        ...
        $.ajax({
            url: g_asiBaseURL + "/ApiCalls/UpdateAuditCriteria",
            type: "POST",
            data: {
