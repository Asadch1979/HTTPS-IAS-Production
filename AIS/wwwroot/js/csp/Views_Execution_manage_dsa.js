            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, v) {
                        $('#manageObsPanel tbody').append('<tr><td class="text-center">' + (i + 1) + '</td><td class="text-center">' + v.audiT_PERIOD + '</td><td>' + v.parA_NO + '</td><td>' + v.annex + '</td><td>' + v.obS_RISK + '</td><td>' + v.obS_GIST + '</td><td>' + v.updateD_BY + '</td><td>' + v.updateD_ON + '</td><td class="text-center"><a data-click="event.preventDefault();ObservationViewerPanel(\'' + i + '\')" href="#" class="text-hover">Update Para Details</a></td><td class="text-center"><a data-click="event.preventDefault();DeleteDuplicatePara(\'' + v.neW_PARA_ID + '\',\'' + v.olD_PARA_ID + '\', \'' + v.p_TYPE_IND + '\')" href="#" class="text-hover">Delete Duplicate Para</a></td></tr>');
                    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.ppNumber > 0) {
                    ...
                    if (g_ele != null) {
                        var $row = $(g_ele).closest('tr');
                        $('#resp_loan_case').val($row.data('loan') || $row.children('td').eq(3).html());
                        $('#resp_loan_amount').val($row.data('lcamount') || $row.children('td').eq(4).html());
                        $('#resp_account_number').val($row.data('account') || $row.children('td').eq(5).html());
                        $('#resp_account_amount').val($row.data('accamount') || $row.children('td').eq(6).html());
                        $('#resp_remarks').val("");
                    }
                }
                else
    ...
    function addResponsibilityToMainTable(action) {
        if (g_respUser[0].ppNumber > 0) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: {
                    'PP_NO': normalizeRequiredInt(g_respUser[0].ppNumber),
                    'LOAN_CASE': normalizeNullableInt($('#resp_loan_case').val()),
                    'LC_AMOUNT': normalizeNullableInt($('#resp_loan_amount').val()),
                    'ACCOUNT_NUMBER': normalizeRequiredInt($('#resp_account_number').val()),
                    'ACC_AMOUNT': normalizeRequiredInt($('#resp_account_amount').val()),
    ...
    function deleteRespRow(e) {
        ...
        var payload = {
            ...
            EMP_NAME: $row.data('emp') || $row.children('td').eq(2).text(),
            NEW_PARA_ID: g_np_id,
            OLD_PARA_ID: g_op_id,
            INDICATOR: g_ind,
            ACTION: 'D'
