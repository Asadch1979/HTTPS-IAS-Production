var iidExceptionReportSetupPage = {
    engId: 0,
    preferredReportId: 0,
    reports: [],
    selectedReportId: 0,
...
$(document).ready(function () {
    ...
    iidExceptionReportSetupPage.engId = parseInt(url.searchParams.get("engId") || url.searchParams.get("complaintId"), 10) || 0;
    iidExceptionReportSetupPage.preferredReportId = parseInt(url.searchParams.get("reportId") || url.searchParams.get("report_id"), 10) || 0;
    ...
    loadReports(iidExceptionReportSetupPage.preferredReportId ? { reportId: iidExceptionReportSetupPage.preferredReportId } : null);
});
...
function loadReports(selectionHint) {
    $.ajax({
        ...
        type: "POST",
        cache: false,
        data: { ENG_ID: iidExceptionReportSetupPage.engId },
        dataType: "json",
        success: function (data) {
            iidExceptionReportSetupPage.reports = normalizeReportList(data);
            var reportIdToSelect = resolveSelectedReportId(selectionHint);
            iidExceptionReportSetupPage.selectedReportId = reportIdToSelect || 0;
            renderReportTable(iidExceptionReportSetupPage.reports);
        ...
        error: function (xhr) {
            iidExceptionReportSetupPage.reports = [];
            renderReportTable([]);
            setSelectedReport(null, { clearMasterForm: false });
...
function renderReportTable(data) {
    ...
    data.forEach(function (item, index) {
        var isSelected = iidExceptionReportSetupPage.selectedReportId === item.ReportId;
...
function resolveSelectedReportId(selectionHint) {
    var currentId = iidExceptionReportSetupPage.selectedReportId;
    ...
    var matchedReport = iidExceptionReportSetupPage.reports.find(function (item) {
...
function hasReport(reportId) {
    return iidExceptionReportSetupPage.reports.some(function (item) {
...
function setSelectedReport(report, options) {
    ...
    if (!report) {
        iidExceptionReportSetupPage.selectedReportId = 0;
    ...
    iidExceptionReportSetupPage.selectedReportId = report.ReportId;
...
function populateReportForm(report) {
    iidExceptionReportSetupPage.reportMode = "edit";
...
function resetReportForm(clearSelection) {
    iidExceptionReportSetupPage.reportMode = "add";
    iidExceptionReportSetupPage.lastLoanStatusBeforeAccount = getDefaultLoanStatusValue();
...
function syncLoanStatusState() {
    ...
    if (isAccountReport) {
        ...
        if (currentValue && currentValue !== "1") {
            iidExceptionReportSetupPage.lastLoanStatusBeforeAccount = currentValue;
        }
    ...
    if (!loanStatus.val() || loanStatus.val() === "1") {
        loanStatus.val(iidExceptionReportSetupPage.lastLoanStatusBeforeAccount || getDefaultLoanStatusValue());
    }
}
...
function saveReport() {
    ...
    var selectionHint = {
        reportId: iidExceptionReportSetupPage.reportMode === "edit" ? iidExceptionReportSetupPage.selectedReportId : 0,
    ...
    $.ajax({
        url: g_asiBaseURL + "/ApiCalls/add_iid_exception_account_report",
        type: "POST",
        data: reportPayload,
        success: function (data) {
            ...
            var successMessage = iidExceptionReportSetupPage.reportMode === "edit"
...
function collectReportPayload() {
    return {
        IND: iidExceptionReportSetupPage.reportMode === "edit" ? "U" : "A",
        REPORT_ID: iidExceptionReportSetupPage.reportMode === "edit" ? iidExceptionReportSetupPage.selectedReportId : 0,
...
function loadFormat(reportId) {
    ...
    $.ajax({
        url: g_asiBaseURL + "/ApiCalls/get_iid_exception_report_format",
        type: "GET",
        cache: false,
        dataType: "json",
...
function saveFormat() {
    if (!iidExceptionReportSetupPage.selectedReportId) {
    ...
    var model = {
        ReportId: iidExceptionReportSetupPage.selectedReportId,
    ...
    if (iidExceptionReportSetupPage.formatMode === "edit") {
        model.FormatId = iidExceptionReportSetupPage.editingFormatId;
    }

    $.ajax({
        url: g_asiBaseURL + (iidExceptionReportSetupPage.formatMode === "edit"
            ? "/ApiCalls/update_iid_exception_report_format"
            : "/ApiCalls/save_iid_exception_report_format"),
        type: "POST",
        ...
        success: function (data) {
            ...
            var successMessage = iidExceptionReportSetupPage.formatMode === "edit"
                ? "Format updated successfully."
                : "Format added successfully.";

            clearFormatForm();
            loadFormat(iidExceptionReportSetupPage.selectedReportId);
            showApiAlert(data, successMessage);
        },
        error: function (xhr) {
...
function beginFormatEdit(rowData) {
    iidExceptionReportSetupPage.formatMode = "edit";
    iidExceptionReportSetupPage.editingFormatId = rowData.FormatId || 0;
...
function clearFormatForm() {
    iidExceptionReportSetupPage.formatMode = "add";
    iidExceptionReportSetupPage.editingFormatId = 0;
...
function getReportById(reportId) {
    return iidExceptionReportSetupPage.reports.find(function (item) {
        return item.ReportId === reportId;
    }) || null;
}
