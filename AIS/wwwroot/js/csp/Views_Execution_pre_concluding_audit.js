...
function getPageData() {
    ...
    function parsePreConNumber(value) {
        var parsed = parseInt(value, 10);
        return Number.isNaN(parsed) ? 0 : parsed;
    }

    function getPreConFieldValue(item, keys) {
        if (!item) {
            return '';
        }
    ...
    function getPreConRiskMetaByAnnexId(annexId) {
        var targetAnnexId = parsePreConNumber(annexId);
        var match = null;

        $.each(g_annexList, function (i, annex) {
            if (parsePreConNumber(getPreConFieldValue(annex, ['ID', 'id'])) === targetAnnexId) {
                match = {
                    riskId: parsePreConNumber(getPreConFieldValue(annex, ['RISK_ID', 'riskId', 'riskid', 'risK_ID'])),
                    riskName: getPreConFieldValue(annex, ['RISK', 'risk', 'DESCRIPTION', 'description'])
                };
                return false;
            }
        });
    ...
    function getPreConRiskMetaByRiskId(riskId) {
        var targetRiskId = parsePreConNumber(riskId);
        var match = null;

        $.each(g_riskList, function (i, risk) {
            if (parsePreConNumber(getPreConFieldValue(risk, ['R_ID', 'r_ID', 'id', 'ID'])) === targetRiskId) {
                match = {
                    riskId: targetRiskId,
                    riskName: getPreConFieldValue(risk, ['DESCRIPTION', 'description', 'RISK', 'risk'])
                };
                return false;
            }
        });
    ...
    function applyPreConRiskDisplay(riskName) {
    ...
    $(document).ready(function () {
     ...
     function viewObservationDetails(obsId, status){
          ...
          if(!g_boPreConReadOnlyMode && status=="Pending"){
           ...
           g_selectedRiskId = parsePreConNumber(data.riskmodeL_ID || data.RISKMODEL_ID);
      ...
      function updateRiskDisplay(riskIdOverride) {
          ...
          var riskMeta = getPreConRiskMetaByAnnexId(annexId);
          var fallbackRiskId = parsePreConNumber(riskIdOverride);

          if (!riskMeta && fallbackRiskId > 0) {
              riskMeta = getPreConRiskMetaByRiskId(fallbackRiskId) || {
                  riskId: fallbackRiskId,
                  riskName: ''
              };
          }

          g_selectedRiskId = riskMeta ? parsePreConNumber(riskMeta.riskId) : fallbackRiskId;
          applyPreConRiskDisplay(riskMeta ? riskMeta.riskName : '');
      }
