        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, row) {
                    $('#auditCriteriaListBox tbody').append("<tr id=" + row.plaN_ID + "><td>" + ++index + "</td><td>" + row.nature + "</td><td>" + row.audiT_PERIOD + "</td><td>" + row.reportinG_OFFICE + "</td><td>" + row.entitY_NAME + "</td><td>" + row.nO_DAYS + "</td><td>" + row.auditeD_BY + "</td><td><a href=\"#\" class=\"text-danger\" data-onclick=\"event.preventDefault();ReferredBackCriteriaRecordFromGrid(" + row.plaN_ID + ");\">Referred Back</a></td><td><a href=\"#\" class=\"text-success\" data-onclick=\"event.preventDefault();ApprovalCriteriaRecordFromGrid(" + row.plaN_ID + ");\">Approve</a></td></tr>");
                });
            },
            dataType: "json",
        });
