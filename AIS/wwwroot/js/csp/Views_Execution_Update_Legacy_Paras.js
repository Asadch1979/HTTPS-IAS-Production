    ...
    function DeleteRecord(e){
        ...
        if ($($(e).closest("tr").find('.ibtnAdd').eq(0)).length > 0) {
            ...
            $.ajax({
                url: g_asiBaseURL + "/ApiCalls/delete_responsibility_of_legacy_para",
                type: "POST",
                data: {
                    'PP_NO': normalizeRequiredInt($($($(respRow).find('td').eq(0)).find('input').eq(0)).val()),
                    'REF_P': g_paraRef,
                    'P_ID': g_paraId
                },
                cache: false,
    ...
    function AddRecord(e)
    {
        ...
        $.ajax({
            url: g_asiBaseURL + "/ApiCalls/add_responsibility_to_legacy_para",
            type: "POST",
            data: {
                'RESP_PP': resP[0],
                'REF_P':g_paraRef,
    ...
    function getLegacyPara()
    {
        ...
        $.ajax({
            url: g_asiBaseURL + "/ApiCalls/get_legacy_paras_for_update",
            type: "POST",
            data: {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (index, item) {
                        ...
                        $('#riskSubGroupSelectBox').append("<option " + className + " value=\"" + item.id + "\"> " + item.suB_V_NAME + "</option> ");
                    });
