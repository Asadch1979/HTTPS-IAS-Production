/*
  ...
  - ARPSE remains independent from PDP in v1 because the current app contract does not carry PDP_ID on ARPSE.
*/
...
CREATE OR REPLACE PACKAGE BODY PKG_COMMERCIAL_AUDIT AS
    ...
    BEGIN
        INSERT INTO T_COM_AUDIT_PDP_HIST (
            ...
            DAC_RECOMMENDATIONS, UPDATED_STATUS, IS_ACTIVE, CREATED_BY, CREATED_ON, UPDATED_BY, UPDATED_ON,
            SNAPSHOT_ACTION, SNAPSHOT_BY, SNAPSHOT_ON
        )
        SELECT
    ...
    BEGIN
        INSERT INTO T_COM_AUDIT_ARPSE_HIST (
            HIST_ID, ARPSE_ID, ARPSE_YEAR_ID, PARA_NO, GIST_OF_PARA, MANAGEMENT_RESPONSE,
            IS_ACTIVE, CREATED_BY, CREATED_ON, UPDATED_BY, UPDATED_ON,
            SNAPSHOT_ACTION, SNAPSHOT_BY, SNAPSHOT_ON
        )
        SELECT
            SEQ_T_COM_AUDIT_ARPSE_HIST.NEXTVAL, ARPSE_ID, ARPSE_YEAR_ID, PARA_NO, GIST_OF_PARA, MANAGEMENT_RESPONSE,
            IS_ACTIVE, CREATED_BY, CREATED_ON, UPDATED_BY, UPDATED_ON,
            P_ACTION, NVL(P_SNAPSHOT_BY, CREATED_BY), SYSDATE
    ...
    BEGIN
        ...
        IF NVL(P_PDP_ID, 0) = 0 THEN
            ...
            INSERT INTO T_COM_AUDIT_PDP (
                ...
                DAC_RECOMMENDATIONS, UPDATED_STATUS, IS_ACTIVE, CREATED_BY, CREATED_ON
            )
            VALUES (
                V_PDP_ID, P_AUDIT_YEAR_ID, TRIM(P_PDP_NO), TRIM(P_GIST_OF_PDP), P_BODY_OF_PDP, P_MANAGEMENT_RESPONSE,
                P_DAC_RECOMMENDATIONS, TRIM(P_UPDATED_STATUS), V_IS_ACTIVE, P_USER_PPNO, SYSDATE
            );
    ...
    BEGIN
        ...
        IF NVL(P_ARPSE_ID, 0) = 0 THEN
            ...
            INSERT INTO T_COM_AUDIT_ARPSE (
                ARPSE_ID, ARPSE_YEAR_ID, PARA_NO, GIST_OF_PARA, MANAGEMENT_RESPONSE,
                IS_ACTIVE, CREATED_BY, CREATED_ON
            )
            VALUES (
                V_ARPSE_ID, P_ARPSE_YEAR_ID, TRIM(P_PARA_NO), TRIM(P_GIST_OF_PARA), P_MANAGEMENT_RESPONSE,
                V_IS_ACTIVE, P_USER_PPNO, SYSDATE
            );
    ...
    BEGIN
        OPEN IO_CURSOR FOR
            SELECT
                ARPSE_ID,
                ARPSE_YEAR_ID,
                TO_CHAR(ARPSE_YEAR_ID) AS ARPSE_YEAR_TEXT,
                PARA_NO,
                GIST_OF_PARA,
                MANAGEMENT_RESPONSE,
                IS_ACTIVE
            FROM T_COM_AUDIT_ARPSE
            WHERE IS_ACTIVE = C_ACTIVE
            ORDER BY ARPSE_YEAR_ID DESC, PARA_NO;
