...
BEGIN
  EXECUTE IMMEDIATE q'~
    ...
    (
      ...
      DELIVERY_MECHANISM    VARCHAR2(100 CHAR) DEFAULT 'PKG_EMAIL.P_ENQUEUE_EMAIL' NOT NULL,
...
USING (
  ...
  SELECT 'MGMT_AUDIT_PARA_STATUS','Management Audit Para Status','Compliance',
    'PKG_AE.P_SUBMITPOSTAUDITCOMPLIANCE_REVIEW','EmailNotification.NotifyManagementAuditParaStatus',
  ...
  SELECT 'MGMT_AUDIT_WEEKLY_PARA_STATUS','Weekly Management Audit Para Decisions','Compliance',
    ...
    'WORKFLOW','N','Previous Monday-Sunday digest sent by ASP.NET, one email per concerned Divisional Head with reporting office/group in CC.' FROM DUAL UNION ALL
...
CREATE OR REPLACE PACKAGE BODY PKG_IAS_NOTIFICATION AS
  ...
  BEGIN
    ...
    PKG_EMAIL.P_ENQUEUE_EMAIL(P_CODE,P_REF1,P_REF2,P_MAIL_TO,P_MAIL_CC,V_SUBJECT,V_BODY,O_EMAIL_ID);
  END;
  ...
  BEGIN
    ...
    IF TRIM(P_MAIL_TO) IS NULL THEN RAISE_APPLICATION_ERROR(-20101,'No TO recipient for '||P_CODE); END IF;
    PKG_EMAIL.P_ENQUEUE_EMAIL(P_CODE,P_REF1,P_REF2,P_MAIL_TO,P_MAIL_CC,V_SUBJECT,P_BODY,O_EMAIL_ID);
  END;
  ...
  BEGIN
    RAISE_APPLICATION_ERROR(-20106,'Weekly notification delivery moved to ASP.NET ManagementAuditWeeklyService.');
  END;
  ...
  BEGIN
    ...
    FOR HD IN (
      ...
      IF TRIM(HD.MAIL_TO) IS NULL THEN
        ...
        CONTINUE;
      END IF;
      SELECT COUNT(*) INTO V_EXISTING FROM T_EMAIL_QUEUE Q
  ...
  BEGIN
    V_REF1:=TO_NUMBER(TO_CHAR(V_DATE,'YYYYMMDD'));
    SELECT COUNT(*) INTO V_EXISTING FROM T_EMAIL_QUEUE
    ...
    FOR E IN (SELECT * FROM (SELECT EMAIL_ID,EVENT_CODE,CREATED_ON,RETRY_COUNT,ERROR_TEXT FROM T_EMAIL_QUEUE
