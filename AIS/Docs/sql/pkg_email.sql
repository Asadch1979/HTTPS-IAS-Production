CREATE OR REPLACE PACKAGE PKG_EMAIL AS
  ...
  PROCEDURE P_ENQUEUE_EMAIL(P_EVENT_CODE IN VARCHAR2, P_REF_ID1 IN NUMBER,
    P_REF_ID2 IN NUMBER, P_MAIL_TO IN VARCHAR2, P_MAIL_CC IN VARCHAR2,
    P_SUBJECT IN VARCHAR2, P_BODY IN CLOB, O_EMAIL_ID OUT NUMBER);
  PROCEDURE P_GET_EMAIL_QUEUE(P_STATUS IN VARCHAR2, P_FROM_DATE IN DATE,
    P_TO_DATE IN DATE, IO_CURSOR OUT SYS_REFCURSOR);
  PROCEDURE P_MARK_EMAIL_SENT(P_EMAIL_ID IN NUMBER);
  PROCEDURE P_MARK_EMAIL_FAILED(P_EMAIL_ID IN NUMBER, P_ERROR_TEXT IN VARCHAR2);
END PKG_EMAIL;
/
CREATE OR REPLACE PACKAGE BODY PKG_EMAIL AS
  ...
  PROCEDURE GET_TRIGGER_LOGS(P_MAX_ROWS IN NUMBER, O_CUR OUT SYS_REFCURSOR) IS
  BEGIN
    OPEN O_CUR FOR
      SELECT *
  ...
  PROCEDURE P_ENQUEUE_EMAIL(P_EVENT_CODE IN VARCHAR2, P_REF_ID1 IN NUMBER,
    P_REF_ID2 IN NUMBER, P_MAIL_TO IN VARCHAR2, P_MAIL_CC IN VARCHAR2,
    P_SUBJECT IN VARCHAR2, P_BODY IN CLOB, O_EMAIL_ID OUT NUMBER) IS
  BEGIN
    ...
    VALUES
      (O_EMAIL_ID,TRIM(P_EVENT_CODE),P_REF_ID1,P_REF_ID2,TRIM(P_MAIL_TO),
       TRIM(P_MAIL_CC),P_SUBJECT,P_BODY,'PENDING',SYSDATE,NULL,NULL,0,NULL);
  END P_ENQUEUE_EMAIL;

  PROCEDURE P_GET_EMAIL_QUEUE(P_STATUS IN VARCHAR2, P_FROM_DATE IN DATE,
    P_TO_DATE IN DATE, IO_CURSOR OUT SYS_REFCURSOR) IS
  BEGIN
    OPEN IO_CURSOR FOR
      SELECT EMAIL_ID,EVENT_CODE,REF_ID1,REF_ID2,MAIL_TO,MAIL_CC,SUBJECT,BODY,
             STATUS,CREATED_ON,SENT_ON,ERROR_TEXT
        FROM T_EMAIL_QUEUE
       ...
       ORDER BY CASE UPPER(STATUS) WHEN 'PENDING' THEN 1 WHEN 'FAILED' THEN 2
                WHEN 'SENT' THEN 3 ELSE 4 END, CREATED_ON, EMAIL_ID;
  END P_GET_EMAIL_QUEUE;

  PROCEDURE P_MARK_EMAIL_SENT(P_EMAIL_ID IN NUMBER) IS
  BEGIN
    UPDATE T_EMAIL_QUEUE SET STATUS='SENT',SENT_ON=SYSDATE,ERROR_TEXT=NULL
     WHERE EMAIL_ID=P_EMAIL_ID;
  END P_MARK_EMAIL_SENT;

  PROCEDURE P_MARK_EMAIL_FAILED(P_EMAIL_ID IN NUMBER,P_ERROR_TEXT IN VARCHAR2) IS
  BEGIN
    UPDATE T_EMAIL_QUEUE SET STATUS='FAILED',ERROR_TEXT=SUBSTR(P_ERROR_TEXT,1,2000),
      RETRY_COUNT=NVL(RETRY_COUNT,0)+1 WHERE EMAIL_ID=P_EMAIL_ID;
