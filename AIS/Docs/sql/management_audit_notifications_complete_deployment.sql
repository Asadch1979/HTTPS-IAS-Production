...
BEGIN
  ...
  SELECT COUNT(*) INTO V_COUNT
   ...
   WHERE TABLE_NAME='T_AUDITEE_ENTITIES' AND COLUMN_NAME IN ('ACTIVE','AUDITABLE');
  IF V_COUNT<>2 THEN
    RAISE_APPLICATION_ERROR(-20801,'Expected T_AUDITEE_ENTITIES.ACTIVE and AUDITABLE indicators were not found.');
  END IF;

  SELECT COUNT(*) INTO V_COUNT
    FROM USER_TAB_COLUMNS
   WHERE TABLE_NAME='T_AUDITEE_ENTITIES_MAPING'
     AND COLUMN_NAME IN ('ENTITY_ID','DIV_OFFICE','REPORTING','B_GROUP','STATUS');
  IF V_COUNT<>5 THEN
    RAISE_APPLICATION_ERROR(-20802,'Required explicit Division/Reporting mapping columns were not found.');
  END IF;

  SELECT COUNT(*) INTO V_COUNT FROM USER_OBJECTS WHERE OBJECT_NAME='PKG_LG' AND STATUS='VALID';
  IF V_COUNT=0 THEN
    RAISE_APPLICATION_ERROR(-20803,'Existing IAS error logger PKG_LG is not valid.');
  END IF;

  DBMS_OUTPUT.PUT_LINE('Preflight passed. ACTIVE=Y and AUDITABLE=Y are the authoritative operational filters.');
END;
/
...
PROMPT ========================================================================
PROMPT F. SCHEDULER CONFIGURATION
PROMPT ========================================================================
SET DEFINE ON;
DECLARE
...
/* Any legacy standalone log is deliberately left untouched; the queue is the retry source of truth. */

PROMPT ========================================================================
PROMPT G. COMPILATION VALIDATION
PROMPT ========================================================================
DECLARE
  V_OBJECT_COUNT NUMBER; V_INVALID NUMBER; V_ERRORS NUMBER; V_STATUS_TYPE VARCHAR2(30);
  V_JOB_COUNT NUMBER; V_INDEX_COUNT NUMBER;
BEGIN
  ...
  IF V_OBJECT_COUNT<>8 OR V_INVALID>0 OR V_ERRORS>0 OR V_STATUS_TYPE<>'VARCHAR2' OR V_JOB_COUNT<>3 OR V_INDEX_COUNT<>1 THEN
    RAISE_APPLICATION_ERROR(-20811,'Compilation/control validation failed: objects='||V_OBJECT_COUNT||'/8, invalid='||V_INVALID||
      ', errors='||V_ERRORS||', P_STATUS='||V_STATUS_TYPE||', enabled_jobs='||V_JOB_COUNT||', valid_index='||V_INDEX_COUNT);
  END IF;
  DBMS_OUTPUT.PUT_LINE('Compilation/control validation passed. P_GET_EMAIL_QUEUE.P_STATUS=VARCHAR2 / VALID.');
END;
/
...
BEGIN
  SELECT MIN(E.EMAIL) INTO V_TO
    ...
    JOIN V_SERVICE_EMPLOYEEINFO E ON E.PPNO=U.PPNO
   WHERE M.ROLE_ID=1 AND NVL(U.ISACTIVE,'Y')='Y' AND TRIM(E.EMAIL) IS NOT NULL;
  IF V_TO IS NULL THEN RAISE_APPLICATION_ERROR(-20812,'No active Super Admin email for queue validation.'); END IF;
  ...
  IF V_STATUS<>'PENDING' THEN RAISE_APPLICATION_ERROR(-20813,'PENDING transition failed.'); END IF;
  ...
  IF V_STATUS<>'FAILED' OR V_RETRY<1 THEN RAISE_APPLICATION_ERROR(-20814,'FAILED/retry transition failed.'); END IF;
  PKG_INQ.P_MARK_EMAIL_SENT(V_EMAIL_ID);
  SELECT STATUS INTO V_STATUS FROM T_AU_IID_EMAIL_QUEUE WHERE EMAIL_ID=V_EMAIL_ID;
  IF V_STATUS<>'SENT' THEN RAISE_APPLICATION_ERROR(-20815,'SENT transition failed.'); END IF;
