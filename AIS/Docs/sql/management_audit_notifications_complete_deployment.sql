/*
  ...
  Oracle scheduling is retained only for mapping exceptions and notification
  health. The weekly digest is owned by ManagementAuditWeeklyService.
*/
...
(
  SELECT X.ENTITY_ID,
         LISTAGG(X.EMAIL_ADDRESS,';') WITHIN GROUP (ORDER BY X.EMAIL_ADDRESS) PARENT_EMAILS
    FROM (
      SELECT DISTINCT M.ENTITY_ID,P.EMAIL_ADDRESS
...
BEGIN
  EXECUTE IMMEDIATE q'~CREATE TABLE T_IAS_NOTIFY_EXECUTION (
...
SELECT H.HIST_ID,M.DIVISION_ID,M.DIVISION_NAME,M.DIVISION_EMAIL,M.REPORTING_EMAIL,
       ...
       (SELECT MAX(S.COMMENT_ON)
         ...
         WHERE S.COM_ID=H.COM_ID AND S.COM_CYCLE=H.COM_CYCLE
           AND S.COM_STATUS=10 AND S.COMMENT_ON<=H.COMMENT_ON
...
DECLARE
  ...
  V_INDEX_COUNT NUMBER; V_ENABLED_JOBS NUMBER; V_WEEKLY_JOBS NUMBER;
BEGIN
  ...
  SELECT COUNT(*) INTO V_INVALID FROM USER_OBJECTS
   WHERE OBJECT_NAME IN ('PKG_INQ','PKG_AE','PKG_IAS_NOTIFICATION','V_IAS_POST_COMPLIANCE_NOTIFY',
                         'V_IAS_MGMT_AUDIT_NOTIFY_MAP','V_IAS_MGMT_WEEKLY_DATA','T_IAS_NOTIFY_EXECUTION')
     AND OBJECT_TYPE IN ('PACKAGE','PACKAGE BODY','VIEW','TABLE') AND STATUS<>'VALID';
  ...
  SELECT COUNT(*) INTO V_ENABLED_JOBS FROM USER_SCHEDULER_JOBS
   WHERE JOB_NAME IN ('JOB_MGMT_AUDIT_MAPPING_EXCEPT','JOB_IAS_NOTIFICATION_HEALTH')
     AND ENABLED='TRUE';
  SELECT COUNT(*) INTO V_WEEKLY_JOBS FROM USER_SCHEDULER_JOBS
   WHERE JOB_NAME='JOB_MGMT_AUDIT_WEEKLY_NOTIFY'
      OR UPPER(JOB_ACTION) LIKE '%SEND_MGMT_AUDIT_WEEKLY%';

  IF V_OBJECT_COUNT<>10 OR V_INVALID>0 OR V_ERRORS>0 OR V_STATUS_TYPE<>'VARCHAR2'
     OR V_INDEX_COUNT<>1 OR V_ENABLED_JOBS<>0 OR V_WEEKLY_JOBS<>0 THEN
    RAISE_APPLICATION_ERROR(
      -20811,
      'Pre-scheduler validation failed: objects='||V_OBJECT_COUNT||'/10, invalid='||V_INVALID||
      ', errors='||V_ERRORS||', P_STATUS='||V_STATUS_TYPE||
      ', valid_index='||V_INDEX_COUNT||', prematurely_enabled_jobs='||V_ENABLED_JOBS||
      ', retired_weekly_jobs='||V_WEEKLY_JOBS
    );
  END IF;
  DBMS_OUTPUT.PUT_LINE(
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('Scheduler enablement passed: both remaining Oracle jobs are enabled.');
END;
/
...
PROMPT 13. Configured and enabled Friday mapping and health scheduler jobs
SELECT JOB_NAME,ENABLED,STATE,REPEAT_INTERVAL,NEXT_RUN_DATE,LAST_START_DATE,LAST_RUN_DURATION,FAILURE_COUNT
  FROM USER_SCHEDULER_JOBS
 WHERE JOB_NAME IN ('JOB_MGMT_AUDIT_MAPPING_EXCEPT','JOB_IAS_NOTIFICATION_HEALTH')
 ORDER BY JOB_NAME;

PROMPT 14. ASP.NET weekly execution ledger
SELECT EXECUTION_KEY,STATUS,RETRY_COUNT,CREATED_ON,UPDATED_ON,RESPONSE
  FROM T_IAS_NOTIFY_EXECUTION
 WHERE EXECUTION_KEY LIKE 'WEEKLY:%'
 ORDER BY CREATED_ON DESC;
