PROMPT ============================================================
PROMPT IAS USER CONTEXT REDESIGN - VERIFIED AGAINST Tables.txt
PROMPT ============================================================
...
------------------------------------------------------------------------
BEGIN
    EXECUTE IMMEDIATE 'CREATE SEQUENCE SEQ_T_USER_CONTEXT_ASSIGNMENT START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE SEQUENCE SEQ_T_USER_CONTEXT_ASSIGNMENT_HIST START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -955 THEN
            RAISE;
...
BEGIN
    EXECUTE IMMEDIATE q'[
        CREATE TABLE T_USER_CONTEXT_ASSIGNMENT
        (
            USER_CONTEXT_ID        NUMBER(18)      NOT NULL,
            USER_ID                NUMBER(18)      NOT NULL,
            PPNO                   NUMBER(18)      NOT NULL,
            ...
            MODIFIED_ON            DATE,
            CONSTRAINT PK_T_USER_CONTEXT_ASSIGNMENT PRIMARY KEY (USER_CONTEXT_ID),
            CONSTRAINT CK_UCA_IS_DEFAULT CHECK (IS_DEFAULT IN ('Y','N')),
            CONSTRAINT CK_UCA_IS_ACTIVE CHECK (IS_ACTIVE IN ('Y','N')),
            CONSTRAINT CK_UCA_EFFECTIVE_DATES CHECK (EFFECTIVE_TO IS NULL OR EFFECTIVE_FROM IS NULL OR EFFECTIVE_TO >= EFFECTIVE_FROM)
        )
    ]';
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    EXECUTE IMMEDIATE q'[
        CREATE TABLE T_USER_CONTEXT_ASSIGNMENT_HIST
        (
            HIST_ID                NUMBER(18)      NOT NULL,
            USER_CONTEXT_ID        NUMBER(18),
            USER_ID                NUMBER(18)      NOT NULL,
            PPNO                   NUMBER(18)      NOT NULL,
            ROLE_ID                NUMBER(18)      NOT NULL,
            GROUP_ID               NUMBER(18),
            ...
            ACTION_ON              DATE            DEFAULT SYSDATE NOT NULL,
            CONSTRAINT PK_T_USER_CONTEXT_ASSIGNMENT_H PRIMARY KEY (HIST_ID),
            CONSTRAINT CK_UCAH_IS_DEFAULT CHECK (IS_DEFAULT IN ('Y','N')),
            CONSTRAINT CK_UCAH_IS_ACTIVE CHECK (IS_ACTIVE IN ('Y','N'))
        )
    ]';
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    EXECUTE IMMEDIATE 'ALTER TABLE T_USER_SESSION ADD USER_CONTEXT_ID NUMBER(18)';
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_UCA_USER_ID ON T_USER_CONTEXT_ASSIGNMENT (USER_ID)';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_UCA_PPNO ON T_USER_CONTEXT_ASSIGNMENT (PPNO)';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_UCA_USER_ACTIVE ON T_USER_CONTEXT_ASSIGNMENT (USER_ID, IS_ACTIVE)';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_UCA_ENTITY_ROLE ON T_USER_CONTEXT_ASSIGNMENT (ENTITY_ID, ROLE_ID)';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN
    EXECUTE IMMEDIATE 'CREATE INDEX IDX_T_USER_SESSION_CTX ON T_USER_SESSION (USER_CONTEXT_ID)';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN
    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX UK_UCA_USER_ROLE_ENTITY ON T_USER_CONTEXT_ASSIGNMENT (USER_ID, ROLE_ID, ENTITY_ID, NVL(GROUP_ID,-1))';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/

------------------------------------------------------------------------
-- 2. USER CONTEXT PACKAGE
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- 3. EXISTING PACKAGE SAFETY NOTE
------------------------------------------------------------------------
-- This script intentionally does not recreate PKG_AD or PKG_LG. Replacing
-- those package specs/bodies would drop existing production procedures.
-- The application calls PKG_USER_CONTEXT directly for redesigned context
-- functions.

------------------------------------------------------------------------
-- 4. OPTIONAL BACKFILL EXECUTION BLOCK
------------------------------------------------------------------------
DECLARE
    V_STATUS  VARCHAR2(100);
    V_MESSAGE VARCHAR2(4000);
BEGIN
    PKG_USER_CONTEXT.P_BACKFILL_USER_CONTEXTS
    (
        P_CREATED_BY => 'DEPLOYMENT',
        O_STATUS     => V_STATUS,
        O_MESSAGE    => V_MESSAGE
    );
